...
%list = ();
$sqll = "select * from LISTEN_LARGE";
$sthh = $dbh->prepare($sqll);
$sthh->execute();
$temp = "";
while (my $reff = $sthh->fetchrow_hashref()) { 
    	my $userr = $reff->{'Speaker'};
    	#print $userr, "\n";
    	my $listenerr = $reff->{'Listener'};
    	#print $listenerr, "\n";
    	$list{$userr} = $list{$userr}." ".$listenerr;
    	print $userr, "\n", $list{$userr}, "\n";
}
...
foreach $bleat_file (@bleats) {
	...
	close B;
	my $rows = $dbh->do("INSERT INTO BLEATS_LARGE (ID, Time, Username, Bleat, ReplyTo, Latitude, Longtitude, Attachment, Ban, Listener) VALUES ('$id', '$time', '$username', '$bleat', '$reply_to', '$latitude', '$longtitude', null, '0', '$list{$username}')");
...
#create table BLEATS_LARGE (ID varchar(50) Primary Key, Time varchar(50), Username varchar(50), Bleat varchar(200), ReplyTo varchar(50), Latitude varchar(50), Longtitude varchar(50), Attachment varchar(200), Ban varchar(50), Listener varchar(500));
